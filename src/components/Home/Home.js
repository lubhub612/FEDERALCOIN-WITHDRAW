...
export default function Home() {
  ...
  const [withdrawUsdtValue, setWithdrawUSDTValue] = useState('');
  ...
  const handleWithdraw = async () => {
    if (window.ethereum.networkVersion !== '97'){
      return toast.error('Please connect to Arbitrum Network');
    }
 ...
 /*   if (withdrawValue > userWithdrawBalance) {
    ...
    try {
      ...
      setShowDanger(false);
        ...
        formData.append('amount', withdrawValue);
         let withdrawApi = await axios.post(`https://federalcoin.social/dashboard/api/coin_redeem.php`, formData).then(async (res, err) => {
            if (res) {
             console.log(res)
             const withdraw = await MLM._withdrawCoin(Number(1), ethers.utils.parseEther(withdrawValue)  );
      ...
      let withdrawApi = await axios.post(`https://federalcoin.social/dashboard/api/redeem.php`, formData).then(async (res, err) => {
            if (res) {
             console.log(res)
             const withdraw = await MLM._withdrawCoin(Number(0),  ethers.utils.parseEther(withdrawUsdtValue)  );
        const waitforTx = await withdraw.wait();
        if (waitforTx) {
          setHandleWithdrawLoader(false);
          toast.success('Withdraw successful.');
  ...
  const  getEstimateWithdraw = async (val) => {
    console.log("🚀 ~ getEstimateToken ~ val", val)
    if (val > 0) {
      if(tokenValue === 'USDT') {
      ...
      setWithdrawUSDTValue(balUSDTF.toString())
    } else {
      setWithdrawValue(val);
    }
  }
            ...
            <div className="col pb-4 ">
              <div className="row ">
                ...
                <div className="col-12 ">
                  ...
                  >
                    (My Balance) - ( {tokenValue === 'USDT' ? userWithdrawLimitBalance : userWithdrawTokenLimitBalance }
                              {   "FDR"})
                  </p>
                </div>
              </div>
              <div className="row  mx-2 ">
                 ...
                 <div className='col d-flex '>
                      ...
                      <input
                            ...
                            type="text"
                            placeholder="Enter FDR Token Value"
                            aria-label="default input example"
                            value={withdrawValue}
                            onChange={(e) => {
                              getEstimateWithdraw(e.target.value)
              ...
              {/* <div className="row  mx-2">
                      ...
                      <div className="row  mx-2">
                        ...
                        <p
                          ...
                          >
                            Withdraw : {estimateValue ?? '0'} {tokenValue}
                          </p>
                          </div>
                      </div>
            </div>
